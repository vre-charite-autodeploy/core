...
_logger = SrvLoggerFactory('ldap_client').get_logger()
...
class LdapClient:
    ...
    def connect(self, dn_code):
        ...
        self.dn = ','.join([
            f'cn={ConfigClass.AD_PROJECT_GROUP_PREFIX}-{dn_code}',
            f'ou=Gruppen',
            f'ou={ConfigClass.LDAP_OU}',
            f'dc={ConfigClass.LDAP_DC1}',
            f'dc={ConfigClass.LDAP_DC2}',
        ])
        _logger.info(self.dn)
        self.objectclass = [ConfigClass.LDAP_objectclass.encode('utf-8')]
        self.conn.set_option(ldap.OPT_X_TLS_CACERTFILE, '/tmp/ldapcrt.pem')
        self.conn.set_option(ldap.OPT_X_TLS_NEWCTX, 0)
        self.conn.start_tls_s()

        self.conn.simple_bind_s(ConfigClass.LDAP_ADMIN_DN, ConfigClass.LDAP_ADMIN_SECRET)
